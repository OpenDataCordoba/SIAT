...
public final class AdministrarInformeProcedimientoDAction extends BaseDispatchAction {

	private Log log = LogFactory.getLog(AdministrarInformeProcedimientoDAction.class);
	...
	public ActionForward inicializar(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (navModel.getAct().equals(CyqConstants.ACT_INFORME)) {
				stringServicio = "getProcedimientoAdapterForBaja(userSession)";
				procedimientoAdapterVO = CyqServiceLocator.getConcursoyQuiebraService().getProcedimientoAdapterForInforme(userSession, commonKey);
				actionForward = mapping.findForward(CyqConstants.FWD_PROCEDIMIENTO_INFORME_ADAPTER);
			}
	...
	public ActionForward informar(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {

		String funcName = DemodaUtil.currentMethodName();
		if (log.isDebugEnabled()) log.debug("entrando en " + funcName);
		UserSession userSession = canAccess(request, mapping, CyqSecurityConstants.ABM_PROCEDIMIENTO_ENC_CyQ, CyqConstants.ACT_INFORME);		
		if (userSession==null) return forwardErrorSession(request);
		...
		try {
			...
			// llamada al servicio
			ProcedimientoVO procedimientoVO = CyqServiceLocator.getConcursoyQuiebraService().informarProcedimiento(userSession, procedimientoAdapterVO.getProcedimiento());
