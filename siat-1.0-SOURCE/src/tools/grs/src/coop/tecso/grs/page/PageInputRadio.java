...
public class PageInputRadio extends PageTagImp {
	...
	// Constructor
	public PageInputRadio(GrsPageContext context, GrsMap map) {
	...
	public String tag() {
		...
		for(HashMap<Object, Object> option: options){
			i++;	
			String checked = "";
			String value = (String) option.get("value").toString();
			String label = (String) option.get("label").toString();

			sb.append("<").append(TAG_NAME).append(" ");

			sb.append(attribute(TYPE_ATTRIBUTE,"radio")); 
