//Reporte Reporte Deudas Cuentas Seleccion
//Mantis 0007907: Prod. I - GRS -> Reporte de Convenios Caducos 
//Fecha: 11/05/2011
...
Siat.ReporteDeudasSeleccion = new function() {
	...
	var SqlIntoTempinal = "INTO TEMP tmp_#m";
	...
	var SqlFiltroViaDeuda = "AND (idviadeuda = #i) ";
	var SqlUnionFinal = "(SELECT * FROM tmp_#m UNION SELECT * FROM tmp_#m) INTO TEMP tmp_final ";
	var	SqlOut = "SELECT * FROM tmp_final";

	// processDeudasSeleccion 
	// Gobierna el proceso del reporte.	
	var processDeudasSeleccion = function(param) {
		...
		Sql.exec(SqlUnionFinal, "gde_deudaadmin", "gde_deudajudicial");
	}
	...
	var outputReport = function (param) {
		...
		if(param.reporte == "csv"){
			Adp.log("\nIniciando construccion Reporte Deudas segun Seleccion\n");

			var filename = Adp.makeFileName("ReporteDeudas", "csv");  
			var filepath = Adp.addFile(filename, "Reporte Deudas", "Reporte Deudas segun Seleccion");
	...
	this.main = function() {
		...
		processDeudasSeleccion(params);
		outputReport(params);
		Sql.close();
	}


}// end ReporteDeudasSeleccion

//start
Siat.ReporteDeudasSeleccion.main();
