...
public class LiqFormConvenioAdapter extends SiatAdapterModel {

	public static final String NAME = "liqFormConvenioAdapterVO";
	...
	// Constructores
	public LiqFormConvenioAdapter() {
		super(GdeSecurityConstants.MTD_FORMALIZAR_CONVENIO);		
	}
	...
	public void calcularTotalHistorio(){
		...
		for (LiqDeudaVO liqDeuda:(List<LiqDeudaVO>)this.getListDeuda()){
	...
	public String getStrPeriodosDeudaView(){
		...
		// Convenios Vias Originales
		if (ModelUtil.isNullOrEmpty(getProcedimiento())){
			for(LiqDeudaVO liqDeuda: (List<LiqDeudaVO>) this.getListDeuda()){
				ret += liqDeuda.getPeriodoDeuda() + " ";
			}
		// Convenios Cyq	
		} else {
			for(LiqDeudaPrivilegioVO liqDeuda: (List<LiqDeudaPrivilegioVO>) getListDeuda()){
				ret += liqDeuda.getDesRecurso() + " - " + liqDeuda.getNumeroCuenta() + " - " + liqDeuda.getDesTipoPrivilegio() + ",  ";
			}
		}
	...
	public Double calcularTotales(){
		...
		totalGeneral = 0D;
		totalEspecial = 0D;
		totalQuirografario= 0D;
		
		// Totalizacion de deuda en CyQ
		for (LiqDeudaPrivilegioVO deudaPrivilegio:(List<LiqDeudaPrivilegioVO>)this.getListDeuda()){
