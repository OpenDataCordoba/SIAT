...
import ar.gov.rosario.siat.gde.buss.bean.AuxImporteRecaudarReport;
...
/**
 * Genera el Reporte de Importes a Recaudar por Planes en forma desconectada.
 * 
 * @author Tecso Coop. Ltda.
 */
public class ReporteImporteRecaudarWorker implements AdpWorker {

	private static Logger log = Logger.getLogger(ReporteImporteRecaudarWorker.class);
	...
	/**
	 *  Generar Reporte de Importe a Recaudar para Planes (PDF).
	 * 
	 * @param adpRun
	 * @throws Exception
	 */
	public void generarReporte(AdpRun adpRun) throws Exception{
		...
		try {
			...
			AuxImporteRecaudarReport importeRecaudarReport = new AuxImporteRecaudarReport();
			
			// Agregado para CdC procurador.
			Procurador procurador = Procurador.getByIdNull(idProcurador);
			if(procurador != null){
				importeRecaudarReport.setProcurador(procurador);				
			}
			
			Recurso recurso = Recurso.getByIdNull(idRecurso);
			importeRecaudarReport.setRecurso(recurso);				
			
			Plan plan = Plan.getByIdNull(idPlan);
			importeRecaudarReport.setPlan(plan);	
			
			importeRecaudarReport.setFechaVencimientoDesde(fechaDesde);
			importeRecaudarReport.setFechaVencimientoHasta(fechaHasta);
			importeRecaudarReport.setUserId(userId);
			importeRecaudarReport.setUserName(userName);
			
			// Aqui se resuelve la busqueda y se genera el archivo resultado
			importeRecaudarReport = Plan.generarPdfForReport(importeRecaudarReport);  
			...
			String fileName = importeRecaudarReport.getReporteGenerado().getFileName();
			String nombre = importeRecaudarReport.getReporteGenerado().getDescripcion();
			String descripcion = importeRecaudarReport.getReporteGenerado().getDescripcion();
