...
/**
 * Procesa la Emision Corregida de Contribucion de Mejoras
 * @author Tecso Coop. Ltda.
 */
public class EmisionCorCdmWorker implements AdpWorker {
	...
	public void execute(AdpRun adpRun) throws Exception {
		...
		if (pasoActual.equals(1L)) { // Paso 1 de la Emision Corregida: Simulacion
			
			String idEmision = adpRun.getParameter(Emision.ADP_PARAM_ID); 
			String idObra = adpRun.getParameter(EmisionCorCdMAdapter.ID_OBRA);
			String idValActTipObr = adpRun.getParameter(EmisionCorCdMAdapter.ID_VALACTTIPOBR);
			this.ejecutarPaso1(adpRun, NumberUtil.getLong(idEmision),
				NumberUtil.getLong(idObra), NumberUtil.getInt(idValActTipObr));
		}
		
		if (pasoActual.equals(2L)) { // Paso 2 de la Emision Corregida: Corregir la Emision
			
			String idEmision = adpRun.getParameter(Emision.ADP_PARAM_ID);
			String idObra = adpRun.getParameter(EmisionCorCdMAdapter.ID_OBRA);
			String idValActTipObr = adpRun.getParameter(EmisionCorCdMAdapter.ID_VALACTTIPOBR);
			//this.ejecutarPaso2(adpRun, NumberUtil.getLong(idEmision),
				//	NumberUtil.getLong(idObra), NumberUtil.getLong(idValActTipObr));
		}
	}

	public boolean validatePaso1(Obra obra) throws Exception {

		// Verificamos que la Obra este en estado: ACTIVA
		if (!obra.getEstadoObra().getId().equals(EstadoObra.ID_ACTIVA)) {
			String descripcion = "La obra no esta Emitida";
		...
		for (PlanillaCuadra planillaCuadra:obra.getListPlanillaCuadra()) {
			// Verificamos que la planilla esta en estado: EMITIDA
			if (!planillaCuadra.getEstPlaCua().getId().equals(EstPlaCua.ID_ENVIADA_A_EMISION) ||
				!planillaCuadra.getEstPlaCua().getId().equals(EstPlaCua.ID_EMITIDA)) {
				String descripcion = "El estado de la planilla numero " 
						+ planillaCuadra.getId() + " no es: ENVIADA A EMISION o EMITIDA";
				addError(descripcion);
			}
			...
			if (ListUtil.isNullOrEmpty(planillaCuadra.getListPlaCuaDet())) {
				String descripcion = "La planilla numero " + planillaCuadra.getId() + " no tiene detalles cargados"; 
				addError(descripcion);
			}
	...
	public void ejecutarPaso1(AdpRun adpRun, Long idEmision, 
		Long idObra, Integer valActTipObr) throws Exception {
