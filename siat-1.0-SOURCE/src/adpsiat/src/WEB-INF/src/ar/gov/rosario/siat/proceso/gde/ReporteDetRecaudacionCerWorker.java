...
/**
 * Genera el Reporte Detallado de Recaudacion Cer por Sector.
 * 
 * @author Tecso Coop. Ltda.
 */
public class ReporteDetRecaudacionCerWorker implements AdpWorker {

	private static Logger log = Logger.getLogger(ReporteDetRecaudacionCerWorker.class);
	...
	public void generarReporte(AdpRun adpRun) throws Exception{
		...
		try {
			...
			String nombre = "Reporte Detallado Total Recaudaci�n Cer";
	...
	public AuxContribuyenteCerReport generarPDF4Report(AuxContribuyenteCerReport auxContribuyenteCerReport, AdpRun adpRun) throws Exception {
		...
		Long idSector;
		HashMap<String, Totales> listadoResultado = new HashMap<String, Totales>();
		...
		List<Contribuyente> listContribuyentebyAct =new ArrayList<Contribuyente>();
		List<DomAtrVal> listSectorActividades = new ArrayList<DomAtrVal>();
	
		for(ConAtrValSec conAtrValSec:listConAtrValSec){
			idSector=conAtrValSec.getId();
			listSectorActividades =DefDAOFactory.getDomAtrValDAO().getListByActividad(conAtrValSec);
			totales = new Totales();
			totales.participacionSector= 0D;
			for(DomAtrVal domAtrVal:listSectorActividades){
				listContribuyentebyAct = PadDAOFactory.getContribuyenteDAO().getContribuyenteCerByAct(domAtrVal, listIdContribuyente, auxContribuyenteCerReport.getFechaReporte());
				totales.cant += listContribuyentebyAct.size(); 
				
				for(Contribuyente contribuyenteSec:listContribuyentebyAct){
				 
					if (listadoResultado.containsKey(idSector.toString()+"-"+ domAtrVal.getDesValor())){
						totales = listadoResultado.get(idSector.toString()+"-"+ domAtrVal.getDesValor());
					} else {
						totales.participacionActividad = 0D;  
					}
					...
					totales.participacionActividad+= totales.totalPago;
					listadoResultado.put(idSector.toString()+"-"+ domAtrVal.getDesValor(), totales);
				}
			}
		...
		indiceArchivo++;
		String fileName = "reporteDetRecaudacionCer_"+indiceArchivo+".csv";
		...
		// --> Filtros ingresados
		buffer.write("Reporte Detalle Recaudaci�n CER");
		buffer.newLine();
		...
		buffer.write(", ");
		buffer.write(", SECTOR");
		buffer.write(", ACTIVIDAD PRINCIPAL");
		buffer.write(", CANTIDAD CONTRIBUYENTES"); 
		buffer.write(", INGRESOS");
		buffer.write(", PARTICIPACION DEL SECTOR");
		buffer.write(", PARTICIPACION DEL CER");
		buffer.newLine();
		...
		Integer cant = 0;
		Double total = 0D;
		Double participacionSector = 0D;
		Double participacionActividad = 0D;
		Double participacionSectorTotal = 0D;
		Double participacionActividadTotal = 0D;
		
		String idSectorAct = "";
		String [] sectorAct = null;
		while (iter.hasNext()) {
			resultadoVacio = false;
			Map.Entry entry = (Map.Entry) iter.next();
			Totales totales2 =(Totales)entry.getValue();
			cant+= totales2.getCant();
			total+= totales2.getTotalPago();
       }
		
		while (itera.hasNext()) {
			resultadoVacio = false;
			Map.Entry entry = (Map.Entry) itera.next();
			Totales totales3 =(Totales)entry.getValue();
			idSectorAct =  (String) entry.getKey();
			
			sectorAct = idSectorAct.split("-",2);
			
			String sector = sectorAct[0];
			String act = sectorAct[1];
			log.debug("Sector: " +totales3.participacionSector+
					" Actividad: " +totales3.participacionActividad
					);
			participacionSector=(totales3.participacionSector*100)/total;
			participacionActividad=(totales3.participacionActividad*100)/(totales3.participacionSector);
			log.debug("participacionSector: " +participacionSector+
					" participacionActividad: " +participacionActividad
					);
		    participacionActividadTotal += participacionActividad;
			participacionSectorTotal += participacionSector;
			buffer.write(", ");
			// Sector
			buffer.write(", " + ConAtrValSec.getByIdNull(new Long(sector)).getSector());
			// Actividad
			buffer.write(", " + act);
			// Cant de contribuyentes CER
			buffer.write(", " +  totales3.getCant());
			// Total en $
			buffer.write(", " +  totales3.getTotalPago());
			// PARTICIPACION Sector
			buffer.write(", " +  participacionActividad+"%");
			// PARTICIPACION CER
			buffer.write(", " +  participacionSector+"%");
			buffer.newLine();
		}
		// TOTALES
		...
		buffer.write(", " +  cant);
		buffer.write(", " +  total);
		buffer.write(", " +  participacionActividadTotal+"%");
		buffer.write(", " +  participacionSectorTotal+"%");
		buffer.newLine();
		// --> Resultado vacio
		if(resultadoVacio){
			// Sin resultados
	...
	public class Totales {
		...
		public Double getParticipacionActividad() {
			return participacionActividad;
		}
		public void setParticipacionActividad(Double participacionActividad) {
			this.participacionActividad = participacionActividad;
		}
