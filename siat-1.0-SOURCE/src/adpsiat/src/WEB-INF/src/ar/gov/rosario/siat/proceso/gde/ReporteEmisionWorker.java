...
/**
 * Genera el Reporte de Emision por Recurso en forma desconectada.
 * 
 * @author Tecso Coop. Ltda.
 */
public class ReporteEmisionWorker implements AdpWorker {

	private static Logger log = Logger.getLogger(ReporteEmisionWorker.class);
	...
	/**
	 *  Generar Reporte de totales de emision
	 * 
	 * @param adpRun
	 * @throws Exception
	 */
	public void generarReporte(AdpRun adpRun) throws Exception{
		...
		try {
			...
			AdpParameter paramIdReclaDeu = adpRun.getAdpParameter(ID_RECCLADEU_PARAM);
			...
			Long idRecClaDeu = NumberUtil.getLong(paramIdReclaDeu.getValue());
			...
			AuxEmisionReport auxEmisionReport = new AuxEmisionReport();
			
			auxEmisionReport.setRecurso(Recurso.getByIdNull(idRecurso));			
			auxEmisionReport.setFechaDesde(fechaDesde);
			auxEmisionReport.setFechaHasta(fechaHasta);
			auxEmisionReport.setRecclaDeu(RecClaDeu.getByIdNull(idRecClaDeu));
			auxEmisionReport.setUserName(userName);
			auxEmisionReport.setUserId(userId);
			
			// Aqui se resuelve la busqueda y se genera el archivo resultado
			auxEmisionReport = GdeDAOFactory.getDeudaDAO().generarPDF4Report(auxEmisionReport);  
			
			if(auxEmisionReport.hasError()){
				adpRun.changeState(AdpRunState.FIN_ERROR, "Error al generar el reporte.", false);
            	adpRun.logError("Error al generar el reporte.");
            	return;
			}
			
			String fileName = auxEmisionReport.getReporteGenerado().getFileName();
			String nombre = auxEmisionReport.getReporteGenerado().getDescripcion();
			String descripcion = auxEmisionReport.getReporteGenerado().getDescripcion();
