 ...
 */
public class ReporteRecaudadoWorker implements AdpWorker {

	private static Logger log = Logger.getLogger(ReporteRecaudadoWorker.class);
	...
	public void generarReporte(AdpRun adpRun) throws Exception{
		...
		try {
			...
			AdpParameter paramFechaPagoHasta = adpRun.getAdpParameter(FECHA_PAGO_HASTA_PARAM);
			
			Long idRecurso = NumberUtil.getLong(paramIdRecurso.getValue());
			Date fechaPagoHasta = DateUtil.getDate(paramFechaPagoHasta.getValue(), DateUtil.ddSMMSYYYY_MASK);
			...
			AuxRecaudadoReport auxRecaudadoReport = new AuxRecaudadoReport();
			
			auxRecaudadoReport.setRecurso(Recurso.getByIdNull(idRecurso));			
			auxRecaudadoReport.setFechaDesde(fechaDesde);
			auxRecaudadoReport.setFechaHasta(fechaHasta);
			auxRecaudadoReport.setFechaPagoHasta(fechaPagoHasta);
			auxRecaudadoReport.setUserName(userName);
			auxRecaudadoReport.setUserId(userId);
						
			// Aqui se resuelve la busqueda y se genera el archivo resultado
			auxRecaudadoReport = GdeDAOFactory.getDeudaJudicialDAO().generarPDF4Report(auxRecaudadoReport);  
			
			if(auxRecaudadoReport.hasError()){
				adpRun.changeState(AdpRunState.FIN_ERROR, "Error al generar el reporte.", false);
            	adpRun.logError("Error al generar el reporte.");
            	return;
			}
			
			String fileName = auxRecaudadoReport.getReporteGenerado().getFileName();
			String nombre = auxRecaudadoReport.getReporteGenerado().getDescripcion();
			String descripcion = auxRecaudadoReport.getReporteGenerado().getDescripcion();
