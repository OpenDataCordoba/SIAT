...
public class LiqDeudaConvenioContainer {
	...
	// Constructor
	/**
	 * Se construyer el container con una lista de deuda y una fecha de formalizacion.
	 * 
	 */
	public LiqDeudaConvenioContainer(List<Deuda> listDeuda, Date fechaFormalizacion){
		
		setListDeuda(listDeuda);
		listConvenioDeuda = new ArrayList<ConvenioDeuda>();
		
		this.viaDeuda=listDeuda.get(0).getViaDeuda();
		for (Deuda deuda:listDeuda){
			...
			convenioDeuda.setFecVenDeuda(deuda.getFechaVencimiento());
	...
	public void setListDeuda(List<Deuda> listDeuda) {
	...
	/**
	 * Realiza la actualizacion de los registros de deuda seleccionada
	 * aplicando si corresponde descuentos al Capital y/o a la Actualizacion.
	 *  
	 * @author Cristian
	 */
	public void calcularActualizacion(){
		...
		for (ConvenioDeuda convenioDeuda:getListConvenioDeuda()){
			...
			try{
				...
				boolean exentaAct;
				if (this.viaDeuda.getId().longValue()==ViaDeuda.ID_VIA_ADMIN){
					exentaAct = convenioDeuda.getDeuda().getCuenta().exentaActualizacion(getFechaFormalizacion(), 
							convenioDeuda.getDeuda().getFechaVencimiento());
				}else{
					exentaAct=convenioDeuda.getDeudaFirstJud().getCuenta().exentaActualizacion(getFechaFormalizacion(), 
							convenioDeuda.getDeuda().getFechaVencimiento());
				}
				log.debug(" ###  LiqDeudaConvenioContainer - calcularActualizacion: " + exentaAct);
				
				// Llamar a la actualizacion de fede
				DeudaAct deudaAct = ActualizaDeuda.actualizar(getFechaFormalizacion(), 
																convenioDeuda.getFecVenDeuda(),
				...
				convenioDeuda.setActualizacion(recargo);
